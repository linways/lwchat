...
// Per-person accent color (Google palette). Picked deterministically from the
// name so each member's card shows a different-colored date banner — easy to
// tell apart when several cards are posted back-to-back.
const ACCENTS = ["#1a73e8", "#188038", "#d93025", "#9334e6", "#e8710a", "#12808a", "#c5221f", "#7b1fa2"];
...
function buildStandupCard(buckets, meta) {
  const sections = [];
  // Colored, bold date line as the first widget — per-person accent color makes
  // back-to-back cards distinguishable; the name stays the (large) card header.
  if (meta.dateLabel) {
    sections.push({
      widgets: [{ textParagraph: { text: `<b><font color="${accentColor(meta.title)}">📅 ${escapeHtml(meta.dateLabel)}</font></b>` } }],
    });
  }
  const counts = {};
