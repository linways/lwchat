...
async function cmdDm(userArg, message, json) {
  ...
  if (!userId) {
    fail(
      `Could not resolve '${userArg}' to a Chat user. Use an email (alice@example.com) or a users/<id>, or run \`lwchat members\` to see known names.`,
      null,
      json
    );
  }
...
async function buildSpaceMemberMap(spaceId) {
  ...
  // removed the legacy annotation-scrape fallback — every space member
