...
async function cmdReply(issueId, message, spaceAlias, imageArg, json) {
  const locations = await resolveLocations(issueId);
  const aliases = Object.keys(locations);
  ...
  const options = await resolveImageOption(imageArg, entry.space, json);
  const result = await sendMessage(entry.space, entry.thread, resolved, options);

  if (json) {
    out({ ok: true, issue_id: issueId, space_alias: entry.space_alias, message_name: result.name, resolved_text: resolved, image: imageArg || null }, true);
  } else {
    if (resolved !== message) console.log(`Mentions resolved: ${resolved}`);
    const withImg = imageArg ? ` (with image: ${imageArg})` : "";
...
// Resolve a value passed to --image into the options object sendMessage /
...
// `lwchat reply <id> "see screenshot" --image bug.png` gets the same
// upload-and-attach behaviour without surprises.
async function resolveImageOption(imageArg, spaceId, json) {
  if (!imageArg) return {};
  ...
  // The "dangling --image at end of args" case is caught earlier in
  // bin/lwchat.js (it can't reach us here as a string).
  if (imageArg.startsWith("--")) {
    fail(
      `--image needs a value (file path), not another flag (got: ${imageArg}). ` +
  ...
  if (/^https?:\/\//i.test(imageArg)) {
    fail(
      "--image takes a local file path, not a URL. " +
  ...
  if (!existsSync(imageArg)) {
    fail(`File not found: ${imageArg}`, null, json);
  }
  const stat = statSync(imageArg);
  if (stat.isDirectory()) {
    fail(`Expected a file path, got a directory: ${imageArg}`, null, json);
  }
  if (!stat.isFile()) {
    fail(`Not a regular file: ${imageArg}`, null, json);
  }

  try {
    const upload = await uploadAttachment(spaceId, imageArg);
...
async function cmdPost(spaceArg, message, threadName, imageArg, json) {
  const config = await loadConfig();
  ...
  const options = await resolveImageOption(imageArg, resolved.space, json);
...
async function cmdDm(userArg, message, imageArg, json) {
  let userId;
  try {
  ...
  const text = resolveMentions(message, await aggregatedMemberMap());
  const options = await resolveImageOption(imageArg, space.name, json);
  const result = await postToSpace(space.name, text, options);

  if (json) {
    out({ ok: true, user_id: userId, space: space.name, message_name: result.name, resolved_text: text, image: imageArg || null }, true);
  } else {
    if (text !== message) console.log(`Mentions resolved: ${text}`);
    const withImg = imageArg ? ` (with image: ${imageArg})` : "";
