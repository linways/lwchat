import { loadConfig, saveConfig, loadIndex, saveIndex, clearIndex, loadThreadRoots, saveThreadRoots, loadMembers, saveMembers, createBackup, restoreBackup, listBackups, deleteBackup, ME_FILE, DATA_DIR, TOKENS_FILE } from "./config.js";
...
async function cmdStandup(opts, json) {
  ...
  const classified = await mapWithConcurrency([...candidates.entries()], async ([thread, c]) => {
    const res = await listThreadMessages(c.space, thread);
    const msgs = res.messages || [];
    const cls = classifyThread(msgs, myId, cutoff);
    // Root (earliest) message — the thread-starter carries the issue URL + details.
    const root = msgs.reduce((a, b) => (!a || (b.createTime || "") < (a.createTime || "") ? b : a), null);
    const rootText = stripAutoFooter(root?.text || "").replace(/\s+/g, " ").trim();
    // Issue id from the thread index, else recovered from the root's Redmine URL
    // (threads not yet in the index still get a proper id + subject this way).
    ...
    // Subject: prefer the Redmine subject; else the root message text.
