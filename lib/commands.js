...
// short-circuit, so multi-space issues are fully discovered). Bounded because
// each space paginates sequentially (up to page_limit calls) — an unbounded
// fan-out over the full space list is exactly what trips API 429s.
async function scanSpaces(issueId, aliases, config) {
  ...
  for (let i = 0; i < aliases.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(batch.map((alias) => scanOneSpace(issueId, alias, config)));
