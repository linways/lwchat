...
async function cmdReply(issueId, message, spaceAlias, attachArg, json) {
  ...
  if (json) {
    out({ ok: true, issue_id: issueId, space_alias: entry.space_alias, message_name: result.name, resolved_text: resolved, image: attachArg || null }, true);
  } else {
...
async function cmdDm(userArg, message, attachArg, json) {
  ...
  if (json) {
    out({ ok: true, user_id: userId, space: space.name, message_name: result.name, resolved_text: text, image: attachArg || null }, true);
  } else {
    if (text !== message) console.log(`Mentions resolved: ${text}`);
    const withImg = attachArg ? ` (with image: ${attachArg})` : "";
