...
import { listSpaces, listThreadMessages, sendMessage, postToSpace, getOrCreateDmSpace, paginateMessages, buildMemberMap, resolveMentions, getMe, listAllMembers, searchDirectory, peopleBatchGet } from "./chat-api.js";
...
async function resolveUserRef(input) {
  ...
  // Layer 3: org directory.
  try {
    const matches = await searchDirectory(input);
    ...
    // Re-throw ambiguity errors; swallow API failures so we can fall back
    // to the annotation cache without losing functionality.
    if (/matches \d+ people/.test(e.message)) throw e;
  }

  // Layer 4: annotation-cache fallback.
...
// Org-wide People-API directory lookup. Independent of whether the user
// shares any space with the matched person — that's the whole point: this
// is the answer to "who's Akshay K P? give me their users/<id>" when they
// were never @mentioned in a space we cache.
async function cmdDirectory(query, json) {
  if (!query) {
    fail("Usage: lwchat directory <name or email>", null, json);
  }
  let matches;
  try {
    matches = await searchDirectory(query);
  } catch (e) {
    fail(`Directory search failed: ${e.message}`, null, json);
  }

  if (json) {
    out({ ok: true, query, count: matches.length, results: matches }, true);
    return;
  }
...
async function buildSpaceMemberMap(spaceId) {
  ...
  // Layer 1: Directory-backed names. Best-effort — falls through to
  // annotations if the scope was revoked, the API is down, or for users
  // not in the org (Chat apps, external members).
  ...
  // Layer 2: annotation cache (legacy buildMemberMap reads recent messages).
  // Cheap-ish; safe to fall through to.
  let annotationNames = new Map();
  try {
    annotationNames = await buildMemberMap(spaceId);
  } catch {
    // best-effort
  }
  ...
  for (const id of userIds) {
    const name = directoryNames.get(id)?.name || annotationNames.get(id) || id;
    map.set(id, name);
  }
  return map;
}
