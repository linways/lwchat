...
async function cmdPost(spaceArg, message, threadName, imageArg, json) {
  ...
  // --image takes a LOCAL FILE PATH. We upload via the Chat media-upload
  // endpoint first, then attach the returned token to the message.
  //
  // Why not URLs (a question users will ask):
  // Google Chat API rejects card/cardsV2 payloads for messages sent with
  // human-OAuth credentials — "Message cannot have cards for requests
  // carrying human credentials." cardsV2 (which would be the URL-image
  // path) is reserved for Chat apps using service-account tokens, a
  // different product surface. So for lwchat, the only working image
  // path is upload-by-bytes. If you want to share a hosted image's URL,
  // just include it in the text — Chat auto-link-previews most image
  // URLs anyway.
  const options = {};
  if (imageArg) {
    ...
    try {
      const upload = await uploadAttachment(resolved.space, imageArg);
      ...
      options.attachmentRef = { attachmentUploadToken: tokenRef };
    } catch (e) {
