...
async function api(path, opts = {}) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      // No response at all. Safe to retry only idempotent GETs — a POST that
      // timed out may have already created a message (posting is irreversible),
      // so blindly re-sending would duplicate it.
    ...
    // 429 is rejected before processing, so retrying it is safe even for
    // writes. 5xx may have reached the backend, so only retry it for GETs.
