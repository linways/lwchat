...
test("buildStandupCard: dateLabel adds a colored banner first; name-only title; deterministic color", () => {
  const meta = { title: "Sibin Baby", subtitle: "Generated by lwchat", dateLabel: "06 Jun 2026 · last 24h" };
  const a = buildStandupCard(sampleBuckets(), meta);
  const sections = a.cardsV2[0].card.sections;
  // banner + prod_release + working + summary = 4
  assert.equal(sections.length, 4);
  const banner = sections[0].widgets[0].textParagraph.text;
  assert.match(banner, /06 Jun 2026 · last 24h/);
  assert.match(banner, /<font color="#[0-9a-f]{6}">/i);
  assert.equal(a.cardsV2[0].card.header.title, "Sibin Baby"); // name-only header
  const colorOf = (c) => c.cardsV2[0].card.sections[0].widgets[0].textParagraph.text.match(/color="(#[0-9a-f]{6})"/i)[1];
  // same name → same color; different name → different color (distinguishable)
  assert.equal(colorOf(a), colorOf(buildStandupCard(sampleBuckets(), { ...meta })));
  assert.notEqual(colorOf(buildStandupCard(sampleBuckets(), { ...meta, title: "Sreekuttan CS" })), colorOf(a));
});
